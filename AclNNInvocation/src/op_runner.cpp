...
#include "aclnn_calc_weight_for_ue.h"
#include "common.h"
...
bool OpRunner::RunOp()
{
    ...
    INFO_LOG("Execute aclnnCalcWeightForUeGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnCalcWeightForUe(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyEvent(startEvent);
        (void)aclrtDestroyEvent(endEvent);
        (void)aclrtDestroyStream(stream);
    ...
    INFO_LOG("Execute aclnnCalcWeightForUe success");
